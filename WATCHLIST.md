...
|---|---|---|---|
| **Anthropic $30–50B raise at up to ~$950B post** | 🟢 SUPERSEDED | **2026-06-11:** closed as a **$65B Series H at ~$965B post**, days before the **confidential S-1 filing on June 1**. Folded into the IPO row below. | (closed) |
...
| **Anthropic Oct 2026 IPO path** | 🟢 FILED | **2026-06-11:** confidential **S-1 submitted to SEC on June 1** at ~$965B target; **Morgan Stanley + Goldman + JPMorgan** underwriters; revenue run-rate ~$47B (vs ~$10B a year ago); bankers expect **>$60B raised**; financials private until ~15d pre-roadshow | Public S-1 (revenue mix by surface: API / Claude.ai / Enterprise / Solutions/FDE — best org-chart-by-revenue map); roadshow window; first-day pop; first-earnings arc |
...
| **TensorWave $350M Series B at $1.55B — AMD + Magnetar led, Nvidia-free AI cloud** | 🟢 NEW | **2026-06-10:** Co-led AMD Ventures + Magnetar Capital; Maverick Silicon, Nexus VP, Western Frontier follow; AMD MI300X/MI325X + ROCm; ex-Lockheed Skunk Works CEO; explicit refusal of Nvidia hardware | Anchor customer announcements (60–90 d window — esp. publicly named hyperscaler or frontier-lab tenant); whether Nvidia cuts inference pricing in response; ROCm-native MLOps tooling startups (the second-order wedges) |
| **Supabase $500M Series at $10.5B (GIC lead)** | 🟢 NEW | **2026-06-2026 wave:** Postgres + Auth + Storage + Edge Functions + Vector; AI-app builders are the de-facto core user | Hiring volume (SDE/MLE fit for the user profile); AI-feature roadmap; whether the AI-app-builder narrative reframes around Supabase vs Vercel |
| **Suno $400M Series D at $5.4B (Bond lead)** | 🟢 NEW | **2026-06-2026 wave:** AI music generation; IP-licensing thread is the live risk | Music-label lawsuit resolution; consumer-generative-AI valuation discipline if rulings go against |
| **PhysicsX $300M Series C at $2.4B (Temasek lead)** | 🟢 NEW | **2026-06-2026 wave:** Industrial AI / simulation-augmented engineering for aerospace, automotive, energy | F500 industrial customer logos; whether defense-budget tailwind extends to dedicated US fund-led B+ rounds in the category |
| **Flourish $500M (brain-inspired AI architectures)** | 🟡 NEW | **2026-06-2026 wave:** Bet on a non-transformer paradigm; initial round | First public benchmark vs frontier transformers; whether neuro-inspired ties to the attractor-models / fixed-point reasoning thread (2026-05-15) |
| **Microsoft in-house AI models (June 2 unveil)** | 🟡 NEW | **2026-06-02:** Microsoft unveiled internally-developed models to reduce Azure / Copilot dependency on OpenAI API and lower per-token costs for developers | Public model card / API availability; per-token pricing vs OpenAI; whether MS-OAI hosting agreement shifts terms in response |
...
| **Claude Opus 4.8 / next Anthropic flagship** | 🟢 SUPERSEDED | **2026-06-09: Claude Fable 5 + Mythos 5 shipped** — Fable is the public Mythos-class. Opus 4.8 stays as the **safeguard-fallback model** for Fable's flagged queries (cyber / bio / now visibly LLM-research per the June 11 reversal). | (closed) |
| **Claude Fable 5 + Mythos 5 (Mythos-class, public)** | 🟢 NEW | **2026-06-09:** Fable 5 GA — **SWE-Bench Pro 80.3% · FrontierCode 29.3% · Mythos-5 cyber 78%** · pricing **$10/M in · $50/M out** (half the Mythos preview $20/$100) · safeguards route flagged sessions to Opus 4.8 in <5% of cases · available API + Bedrock + Vertex | Adoption curve in agent stacks (Cursor / Replit / Vercel); whether OpenAI / Google cut prices in response; Mythos 5 trusted-access tenant list; how the June 11 visible-safeguards change affects measured fallback rate |
| **Anthropic silent-safeguards reversal → visible-fallback regime** | 🟢 NEW | **2026-06-11:** Anthropic walked back hidden steering-vector + prompt-modification safeguard on Fable 5 for frontier-LLM-research queries (pretraining / distributed training / ML accelerator design) after Simon Willison + Nathan Lambert + Dean Ball + Fortune backlash; statement to Wired: *"we made the wrong tradeoff and we apologize."* Flagged queries now visibly fall back to Opus 4.8 | Whether OpenAI / Google publish parity transparency commitments; whether "safeguard transparency" becomes a buyer requirement in enterprise procurement; visible-safeguard observability as a wedge (eval tools that detect silent degradation across providers); whether US CAISI / EU equivalents codify visibility as a baseline |
...
| **Apple "Extensions" SDK** | 🟢 PARTIAL | **2026-06-08 WWDC:** Apple Intelligence rebuilt around a **custom ~$1B/yr Gemini partnership (~1.2T params)** via Private Cloud Compute; **iOS 27 Agent Extensions for enterprise** previewed; on-device runs Apple's own next-gen foundation models. Anthropic / OpenAI integration via Extensions handoff layer **not committed to a date**. **Flips the Anthropic-as-default-cloud-brain narrative from 2026-05-07.** | When (if ever) Extensions opens cloud-tier model swapping to Anthropic / OpenAI; first developer-facing Agent Extensions API docs; Apple's own Apple AI / Apple Intelligence Engineering hiring volume (the under-priced lane) |
| **Apple ↔ Google Gemini partnership ($1B/yr · 1.2T params · multi-year)** | 🟢 NEW | **2026-06-08 WWDC:** Apple confirmed the deep multi-year deal — custom Apple-tuned Gemini on Private Cloud Compute for Siri's cloud tier; hybrid with Apple Silicon on-device | Whether $1B/yr terms hold past first renewal; whether the partnership constrains Google's own consumer Gemini surfaces; Apple's response if Gemini ships a safety incident (cf. June 11 Anthropic reversal pattern); engineering team size at the Apple ↔ Gemini integration boundary |
...
| **Claude Corps fellowship ($150M / 1,000 fellows / $85K / 12 mo)** | 🟢 NEW | **2026-06-11:** Anthropic + CodePath (employer-of-record) + Social Finance (M&E); cohort 1 = 100 fellows; apps close **July 17**; placements October 2026; eligibility 18+ with <2 yr FT experience, any educational background; host orgs get $10K + free Claude credits | Cohort-1 named-fellow list (acquisition-target signal for nonprofit-vertical startups); host-org distribution (signal about Anthropic's preferred nonprofit verticals); whether cohort-2 scales beyond 100; correlation with eventual Anthropic full-time hiring funnel |
| **OpenAI Codex MCP schema upgrade + cross-vendor MCP convergence** | 🟢 NEW | **2026-06-2026:** Codex added richer MCP tool-schema support + plugin marketplace + web-search-in-code-mode. Second major non-Anthropic stack on MCP (after Chrome 149 WebMCP) | Whether Gemini's Agent SDK / Antigravity adds first-class MCP schema support; first MCP-native startups raising at growth stage; W3C standardization path for MCP-as-protocol |
| **OpenAI sunsets (GPT-4.5 → June 27, o3 → Aug 26)** | 🟡 NEW | **2026-06-2026:** Public sunset windows set; cleaning the SKU deck pre-IPO | Whether o3 / 4.5-dependent enterprise integrations file complaints; whether OpenAI extends windows; the runtime-portability / model-sunset-abstraction wedge for startups |
| **OpenAI Workspace Agents (free until July 6, then credit-priced)** | 🟡 NEW | **2026-06-2026:** Free-period window for cross-vendor experiments | Adoption uptake post-July 6; pricing relative to Claude Code agents; whether enterprise Microsoft 365 buyers see Workspace Agents as Copilot competition |
