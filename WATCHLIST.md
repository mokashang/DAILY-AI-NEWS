...
| **Anthropic IPO path (October 2026 target)** | 🟢 FILED | **2026-09-11:** **S-1 filed** — bookrunners Goldman / Morgan Stanley / JPMorgan; NYSE ticker **ANTH** reserved; TTM revenue ~$46B; **Claude Code disclosed at ~46% of TTM (~$21B TTM)**; segment gross margin >72% for Claude Code and enterprise API; ~$255B contracted compute through 2032 (Colossus + Google TPU); ~4-week roadshow; pricing target late-October 2026 | First-day pop; whether roadshow demand supports the ~$1T narrative; opening-day multiple as the benchmark for OpenAI's Q4 IPO; ~90-day post-IPO alumni founder wave; whether Anthropic gives Claude Code segment-margin disclosure at each earnings report |
| **The IPO wave as an asset-class shift (SpaceX + OpenAI + Anthropic)** | 🟡 | **2026-09-11:** Anthropic S-1 filed today (moves ahead of OpenAI's Q4 target); **Anthropic goes first** = the multiple gets set here, then OpenAI benches against it | ANTH first-day print; OpenAI response filing timing; whether Google's DeepMind carve-out surfaces in the next 90d |
| **Braintrust $85M Series B / $1.1B (eval-infra category leader)** | 🟢 NEW | **2026-09-11:** Sequoia + Greylock + Index; total $145M; positioned as "eval infra for AI teams shipping multi-provider stacks" — model-agnostic SDK + cost trace + regression detection + public leaderboard = **first funded round on the model-fatigue tooling wedge from 2026-09-10/02 §3** | Their OSS release cadence; whether Anthropic / OpenAI bundle a competing SKU; #2 category entrant landing $30M+ inside 90 days |
| **Meta acqui-hires Freeplay (~$180M, eval consolidation)** | 🟢 NEW | **2026-09-11:** ~24 engineers into Muse Spark Applied AI org; product wound down 6 mo; **fourth big lab with internal-first eval stack** (Google Vertex Evaluation + Anthropic "Verdict" internal + OpenAI Evals API + now Meta) | Whether the labs also acqui-hire router startups by Q1 2027; residual independent-category surface (Braintrust + LangSmith + Promptfoo + stealth) |
| **Metronome AI $35M Series A / $300M post (FinOps for LLM workloads)** | 🟢 NEW | **2026-09-11:** Bessemer lead; cost-attribution SaaS specifically for LLM workloads; charge model = % of savings realized | Design-partner logos; whether integrations extend beyond Anthropic + OpenAI + Google |
| **Verdant Robotics $52M Series B / $340M (embodied agri-AI)** | 🟢 NEW | **2026-09-11:** confirms the embodied-AI + vertical-data-moat barbell from 2026-09-10/02 §1 (Instinct + General Intuition + Nexthop) | Adjacent embodied-vertical rounds this month; whether embodied-Muse-Spark becomes a licensing SKU |
| **Audit-log / evidence-integrity infra as a funded category** | 🟡 NEW | **2026-09-11:** post-Apple-v-OpenAI-Rule-26 admission; YC S26 Amber + Longitudinal pivoting in; Salesforce Ventures + Amex Ventures taking pitch meetings; Anthropic posts 3 net-new Legal-Ops-Eng roles (Head of Litigation Response Engineering + Senior Records-Retention Engineer + Discovery Data Platform Lead) | First funded round on the wedge; whether a frontier lab acqui-hires by Q4; enterprise procurement RFPs on retention-hold-as-a-service |
...
| **Apple v OpenAI trade-secrets suit (N.D. Cal., Judge Davila)** | 🟡 NEW | **2026-09-11:** OpenAI files Rule 26 response — admits limited deletion of 3 Slack channel logs (device-team-general / hw-partners-priv / supply-chain-sync) per 90-day retention timer between case filing (Jul) and preservation letter (Aug); OpenAI contests spoliation framing (routine automated timer); Apple's exhibits include ex-Apple employees' Jun lawyer statements as evidence OpenAI knew suit was imminent. **Oct 1 sanctions hearing now on concrete disputed facts.** | Oct 1 hearing outcome (adverse-inference sanctions probability); OpenAI hardware roadmap slippage if sanctions land; downstream on Anthropic + Google internal retention hygiene disclosures; audit-log wedge funding rounds (see 2026-09-11/02 §3) |
| **OpenAI C-suite churn** | 🟡 NEW | **2026-09-11:** CSO Jason Kwon departs; Sarah Friar (CFO) takes strategy interim. **Third named C-suite exit in 6 weeks** (McGrew CRO Aug 4; Deng VP Product late-Aug via Meta counter-offer; Kwon today). Contrasted with Anthropic S-1 filing same day = asymmetric labor market. | Fourth C-suite exit inside 30d = credibility inflection; whether Kwon lands as fund operating partner (early wedge signal); Anthropic Solutions/Applied AI senior req backfill velocity |
